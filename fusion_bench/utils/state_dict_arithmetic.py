...
def randomk_values_mask(M, K=0.7, rescale=True):
    if K > 1:
        K /= 100
    # original_shape = M.shape
    if M.dim() == 1:
        M = M.unsqueeze(0)
    mask = torch.bernoulli(torch.ones_like(M) * K)
    if not rescale:
        return M * mask
    else:
        return M * mask / K

def state_dict_sub(a: _StateDict, b: _StateDict, strict: bool = True, device=None, threshold=None):
    """
    ...
    for k in a:
        if k in b:
            ...
            if threshold is not None:
                diff[k] = randomk_values_mask(diff[k], threshold).squeeze()
    return diff
