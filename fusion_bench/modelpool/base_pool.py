...
class ModelPool(ABC):
    ...
    # def save_model(self, model: nn.Module, path: str, save_tokenizer: bool = False, push_to_hub: bool = False):
    #     """
    ...
    def save_model(
        ...
        """
        Save the model, tokenizer, processor, and feature extractor if available.
        """

        with timeit_context(f"Saving the state dict of model to {path}"):

            # 1. save HF model (works for most nn.Module from transformers)
            model.save_pretrained(path)

            # 2. save tokenizer (existing logic)
            if save_tokenizer:
                if self.has_pretrained:
                    tokenizer = AutoTokenizer.from_pretrained(
                        os.path.expanduser(self.get_model_config("_pretrained_").path)
            ...
            # 3. Save processor (e.g., CLIPProcessor, Qwen2VLProcessor)
            ...
            # 4. Save feature_extractor (e.g., CLIPImageProcessor)
            if hasattr(self, "feature_extractor") and self.feature_extractor is not None:
                try:
                    self.feature_extractor.save_pretrained(path)
                except Exception as e:
                    print(f"[WARNING] Failed to save feature_extractor: {e}")

            # 5. Save image processor for newer models（optional）
            if hasattr(self, "image_processor") and self.image_processor is not None:
                try:
                    self.image_processor.save_pretrained(path)
                except Exception as e:
                    print(f"[WARNING] Failed to save image_processor: {e}")
