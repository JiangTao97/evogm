"""
EvoGM Algorithm (Huawei Ascend NPU).
...
- Persistent workers with NPU device isolation via ASCEND_RT_VISIBLE_DEVICES
"""
...
from .evogm_worker_npu import _gpu_evaluation_worker_v3, _calculate_score
import threading
...
class EvoGMAlgorithm(
    ...
    def _save_large_tensor_dict(self, obj, path: str):
        """Save large tensor dicts via legacy serialization to avoid zip-writer failures on torch_npu."""
        os.makedirs(os.path.dirname(path), exist_ok=True)
    ...
    def _record_memory_trace(self, event: str, task_suffix: Optional[str] = None, extra: Optional[Dict] = None):
        ...
        if hasattr(torch, "npu") and torch.npu.is_available():
            for attr_name, field_name in (
                ("memory_allocated", "npu_memory_allocated_mb"),
                ("memory_reserved", "npu_memory_reserved_mb"),
            ):
                if hasattr(torch.npu, attr_name):
                    try:
                        entry[field_name] = getattr(torch.npu, attr_name)() / (1024 ** 2)
                    except Exception:
                        pass
        if extra:
            entry.update(extra)
    ...
    def _get_available_gpus(self) -> List[int]:
        ...
        gpu_ids = list(range(torch.npu.device_count()))
            ...
            try:
                with torch.no_grad():
                    ...
                    if self.evaluation_backend == "script":
                        ...
                        merged_model = merged_model.to("npu:0")
    ...
    def _generate_candidates(self, dual_gen: nn.Module, population: np.ndarray) -> np.ndarray:
        """Generate new candidate solutions from the current population using the trained generator.

        Override this in subclasses to add diagnostic comparisons.
        """
        with torch.no_grad():
        ...
        return np.clip(new_candidates, -1, 1)

    def _post_iteration_hook(self, **kwargs):
        """Hook called after each iteration's generation and evaluation.

        Override in subclasses to add diagnostic data collection.
    ...
    def run(self, modelpool, taskpool=None):
        ...
        if torch.npu.is_available():
            self.device = torch.device("npu:0")  # Use first GPU for generator
        else:
            self.device = torch.device("cpu")
            log.warning("NPU not available, running on CPU")
            ...
            try:
                ...
                for round_num in range(self.n_rounds):
                    ...
                    for iteration in range(self.max_iter):
                        ...
                        new_candidates = self._generate_candidates(dual_gen, population)

                        new_scores, _ = self._evaluate_population(new_candidates, split="dev", task_name=task_name)

                        # Hook for diagnostic subclasses
                        self._post_iteration_hook(
                            round_num=round_num, iteration=iteration,
                            dual_gen=dual_gen,
                            population=population, scores=scores,
                            winners=winners, losers=losers,
                            winner_scores=winner_scores, loser_scores=loser_scores,
                            new_candidates=new_candidates, new_scores=new_scores,
                            task_name=task_name,
                        )
        ...
        # Return a sentinel dict so cli.py skips re-evaluation.
        # The EvoGM algorithm already evaluates internally and saves results to disk.
        # merged_model is cleared in the finally block above, so returning it would be None.
        return {
            "already_evaluated": True,
            "report": all_task_results,
            "algorithm": "evogm_npu",
        }
