...
- Workers receive pretrained_sd and task_vectors via shared memory (zero-copy)
- No torch.load for TV/pretrained_sd — eliminates 144GB RAM per worker
- On RELOAD, new shared tensors are sent via a dedicated data queue
- Only the GPU model is loaded from disk (unavoidable, ~14.4GB VRAM per worker)
"""
...
def _gpu_evaluation_worker_v3(
    ...
    """
    EvoGM persistent worker with shared memory (GPU).
    ...
    logging.basicConfig(
        level=logging.INFO,
        format=f'[Worker GPU {gpu_id}] %(asctime)s - %(levelname)s - %(message)s',
        force=True
    )
    log = logging.getLogger(__name__)
    ...
    try:
        # =============================
        # Load GPU model (unavoidable disk I/O, ~14.4GB VRAM)
        # =============================
        ...
        while True:
            ...
            try:
                ...
                # Merge: pretrained + total_task_vector (on CPU, then load to GPU)
                if total_task_vector is not None:
                    ...
                    for k, v in pretrained_sd.items():
                        if k in total_task_vector:
                            ...
                            merged_sd[k] = v
                    # load_state_dict handles CPU→GPU transfer in-place
                    # This avoids having both old and new weights on GPU simultaneously
                ...
                # Restore original pretrained weights (CPU→GPU in-place)
